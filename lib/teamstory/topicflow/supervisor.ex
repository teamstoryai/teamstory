defmodule Teamstory.Topicflow.Supervisor do
  use Supervisor

  alias Teamstory.Topicflow.{Healthcheck, Socket, Session, Topic, Antientropy}
  ...
  @impl true
  def init(args) do
    port = Application.get_env(:teamstory, :topicflow_port, 4100)
    topicflow_disabled = Application.get_env(:teamstory, :topicflow_disabled, false)
  ...
  defp memcache_workers do
    if Teamstory.Topicflow.Memcache.enabled?() do
      Enum.map(Teamstory.Topicflow.Memcache.connections(), &memcache_worker(&1))
    else
      []
    end
  end
  ...
  defp memcache_worker(id) do
    hostname = Application.get_env(:teamstory, :topicflow_memcache_hostname, "localhost")
