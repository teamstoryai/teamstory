defmodule Teamstory.Utils.Sourcemaps do
  ...
  def upload_sourcemaps() do
    ...
    js_path = Path.join(:code.priv_dir(:teamstory), "static/js")
    ...
    Enum.each(js_files, fn file ->
      ...
      if File.exists?(source_map_path) do
        form = [
          ...
          {"name[]", Teamstory.base_url <> "/js/" <> file},
          {"name[]", Teamstory.base_url <> "/js/" <> file <> "?vsn=d"},
          {"revision", revision}
        ]
        env = Teamstory.env
