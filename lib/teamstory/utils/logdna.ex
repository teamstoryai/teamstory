defmodule LogDNA do
  ...
  def post(lines, hostname, mac \\ nil, ip \\ nil, tags \\ nil, allow_dev \\ false) do
    key = Application.get_env(:teamstory, :logdna_key)
    allowed = Teamstory.prod?() or Teamstory.staging?() or allow_dev
  ...
  def post(lines) do
    ...
    ip = Node.self() |> to_string |> String.replace("teamstory@", "")
