defmodule TeamstoryWeb.AdminController do
  use TeamstoryWeb, :controller
  require Logger
  import Ecto.Query, warn: false

  action_fallback TeamstoryWeb.FallbackController
  alias Teamstory.{Billing, Invites, Teams, Users, Users.User, Repo, Deleter, Orgs}

  plug TeamstoryWeb.EnsureAdmin
        ...
        case Teams.team_by_uuid(team_id) do
          {:ok, team} ->
            ...
            if user_team do
              ...
              TeamstoryWeb.Endpoint.broadcast("user:#{user.uuid}", "update_teams", %{})
              TeamstoryWeb.Endpoint.broadcast("team:#{team.uuid}", "update_members", %{})
  ...
  def delete_user(conn, %{ "email" => email, "team" => nil }) do
    with {:ok, user} <- Users.find_by_email(email) do
      if TeamstoryWeb.EnsureAdmin.validate_admin(user) == :ok and !Teamstory.dev? do
  ...
  def merge_teams(conn, %{ "team1" => team1, "team2" => team2 } = params) do
    with {:same_team, false} <- {:same_team, team1 == team2},
      ...
      if params["confirm"] do
        ...
        TeamstoryWeb.Endpoint.broadcast("team:#{team2.uuid}", "update_team", %{})
