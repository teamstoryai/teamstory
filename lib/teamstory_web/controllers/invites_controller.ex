defmodule TeamstoryWeb.InvitesController do
  use TeamstoryWeb, :controller
  require Logger

  action_fallback TeamstoryWeb.FallbackController
  alias Teamstory.{Invites, Teams, Teams.UserTeam, Teams.Team}
  ...
  defp validate_receipient_emails(recipients) do
    ...
    |> Enum.map(fn e -> {e, Teamstory.dev? || Teamstory.test? || EmailChecker.valid?(e)} end)
