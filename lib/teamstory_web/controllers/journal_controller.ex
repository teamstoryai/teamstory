defmodule TeamstoryWeb.JournalController do
  ...
  alias Teamstory.{Journal, Projects, Utils}
      ...
      case Redix.command(:redix, ["GET", "summary:" <> prompt_hash]) do
        {:ok, nil} ->
          case Hammer.check_rate("journal:#{user.id}", 60_000, 5) do
            {:allow, _count} ->
              with {:ok, response} <- Teamstory.OpenAI.completions(prompt, "text-curie-001", 150, 0.3) do
                IO.inspect(response)
