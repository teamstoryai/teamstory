defmodule TeamstoryWeb.StorageController do
  use TeamstoryWeb, :controller
  require Logger

  alias ExAws.S3
  alias Teamstory.{Attachments, Projects}
  alias Teamstory.Users.{Avatar, User}
  ...
  def upload_profile_picture(conn, %{ "upload" => upload, "token" => token } = params) do
    with {:ok, user, _} <- Teamstory.Auth.Guardian.resource_from_token(token) do
      ...
      with {:ok, file_name} <- Avatar.store({upload, scope}),
        ...
        if params["team"], do: TeamstoryWeb.Endpoint.broadcast("team:#{params["team"]}", "update_members", %{})
        json conn, %{
          name: file_name,
  ...
  def upload_profile_picture(conn, %{ "upload" => upload }) do
    with user <- Guardian.Plug.current_resource(conn) do
      ...
      with {:ok, file_name} <- Avatar.store({upload, scope}),
        ...
        unless is_nil(team_uuid) do TeamstoryWeb.Endpoint.broadcast("team:#{team_uuid}", "update_members", %{}) end

        put_view(conn, TeamstoryWeb.AuthView) |> render("user.json", user: user)
      else
  ...
  def upload_attachment(conn, %{ "upload" => upload, "token" => token, "project_id" => project_uuid }) do
    with {:ok, user, _} <- Teamstory.Auth.Guardian.resource_from_token(token),
