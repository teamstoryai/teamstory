defmodule TeamstoryWeb.AuthController do
  use TeamstoryWeb, :controller
  require Logger

  alias Teamstory.{Auth, Auth.OAuth, Projects, Orgs, Users, Users.User, Utils}
  ...
  def sign_in(conn, %{"email" => email, "password" => password}) do
    case Users.authenticate_user(email, password) do
      ...
      {:error, reason} ->
        if !Teamstory.test?(), do: Process.sleep(2000)
  ...
  defp create_user(conn, user_attrs) do
    with {:ok, user} <- Users.create_user(user_attrs) do
      Orgs.user_joined(user)
  ...
  defp attrs_from_google_profile(profile, params) do
    ...
    # prefer given name/nickname
    |> Map.merge(Map.get(params, "user") || %{})
    # prefer fetched google id and email
      ...
      {:ok, profile} ->
        case Users.find_by_google_id(profile.id) do
          {:ok, user} ->
            ...
            if Enum.any?(update) do
              Users.update_user(user, update)
            else
              {:ok, user}
            end
  ...
  defp profile_picture_update(user, profile) do
    if not is_nil(profile.profile_img) and
         (is_nil(user.profile_img) or
            (String.contains?(user.profile_img, "googleusercontent.com") and
               profile.profile_img != user.profile_img)) do
      %{profile_img: profile.profile_img}
    else
      %{}
    end
  ...
  def update_user(conn, params) do
    attrs =
      Enum.reduce(["name", "nickname", "email", "password", "timezone"], %{}, fn key, acc ->
        if Map.has_key?(params, key),
          do: Map.put(acc, Macro.underscore(key), params[key]),
          else: acc
      end)
        ...
        cond do
          params["meta"] ->
            if params["meta"]["ce"] do
              TeamstoryWeb.Endpoint.broadcast("user:#{user.uuid}", "update_meta", %{ce: true})
            end
  ...
  def login_success(conn, %{"code" => code} = params) do
    with user <- Guardian.Plug.current_resource(conn) do
      ...
      token = Auth.gen_token(user)
      TeamstoryWeb.Endpoint.broadcast("login:#{code}", "token", %{token: token, payload: payload})
      json(conn, %{success: true})
    end
  end

  # POST /api/v1/exchange_token?fork=true
  def exchange_token(conn, %{"fork" => "true"} = params),
    do: exchange_token(conn, %{params | "fork" => true})
  ...
  def exchange_token(conn, %{"token" => token, "fork" => true}) do
    with {:ok, user, claims} <- Auth.Guardian.resource_from_token(token),
         {:ok, {refresh, %{"exp" => refresh_exp}}, {access, %{"exp" => access_exp}}, fork} <-
           do_fork_token(user, token, claims) do
      json(conn, %{
        access: %{token: access, exp: access_exp},
  ...
  def exchange_token(conn, %{"token" => token}) do
    with {:ok, user, claims} <- Auth.Guardian.resource_from_token(token),
         {:ok, {refresh, %{"exp" => refresh_exp}}, {access, %{"exp" => access_exp}}} <-
           do_exchange_token(user, token, claims) do
      json(conn, %{
        access: %{token: access, exp: access_exp},
        refresh: %{token: refresh, exp: refresh_exp}
      })
